...
# 初始化图片字典
def load_img_dictionary(task_name):
    # 根据 task_name 和 base_dir 动态调整图片目录
    img_path = os.path.join(base_dir, task_name)
    img_dictionary = {}
    # 确保目录存在
    if not os.path.exists(img_path):
        raise FileNotFoundError(f"Directory {img_path} not found")
...
class Player(object):
    def __init__(self, task_name, accuracy=0.8):
        self.taskName = task_name
        super(Player, self).__init__()
        self.accuracy = accuracy
        self.wait_second = 3
        self.img_dictionary = load_img_dictionary(task_name)
